...
from ..core.models import PaperRecord, Citation, Author, Institution
...
class IndependenceClassifier:
    ...
    def __init__(self, author_similarity_threshold: float = 0.8, institution_similarity_threshold: float = 0.9):
    ...
    def _build_institution_network(self, papers: List[PaperRecord]) -> Dict[str, Set[str]]:
        ...
        for paper in papers:
            ...
            for author in paper.authors:
                for institution in author.institutions:
                    normalized_name = self._normalize_institution_name(institution.display_name)
    ...
    def _normalize_institution_name(self, name: str) -> str:
        ...
        remove_words = [
            ...
            'hospital', 'medical', 'health', 'system', 'dept', 'department',
            'faculty', 'division', 'laboratory', 'lab', 'research', 'sciences'
        ]
    ...
    def analyze_citation_patterns(self, papers: List[PaperRecord]) -> Dict[str, Any]:
        ...
        field_patterns = defaultdict(lambda: {
            'papers': 0, 'citations': 0, 'independent': 0, 'self': 0
        })
        ...
        # Year-wise analysis
        year_patterns = defaultdict(lambda: {
            'papers': 0, 'citations': 0, 'independent': 0, 'self': 0
        })
        ...
        analysis = {
            ...
            'overall_independence_ratio': total_independent / total_citations if total_citations > 0 else 0.0,
            'overall_self_citation_ratio': total_self / total_citations if total_citations > 0 else 0.0,
            'field_patterns': dict(field_patterns),
            'year_patterns': dict(year_patterns),
            'highly_independent_papers': [
                paper.id for paper in papers 
                if paper.citation_count and paper.citation_count > 5 
                and paper.independent_citations and paper.independent_citations / paper.citation_count > 0.8
            ],
            'high_self_citation_papers': [
                paper.id for paper in papers 
                if paper.citation_count and paper.citation_count > 5 
                and paper.self_citations and paper.self_citations / paper.citation_count > 0.5
            ]
        }
    ...
    def generate_independence_report(self, papers: List[PaperRecord]) -> Dict[str, Any]:
        ...
        papers_with_high_independence = sum(
            ...
            and paper.independent_citations and paper.independent_citations / paper.citation_count > 0.7
        )
        
        papers_with_citations = sum(1 for paper in papers if paper.citation_count and paper.citation_count > 0)
        
        independence_quality_score = (
            papers_with_high_independence / papers_with_citations * 100 
            if papers_with_citations > 0 else 0
        )
        ...
        report = {
            ...
            'quality_metrics': {
                ...
                'average_independence_ratio': np.mean([
                    ...
                    for paper in papers 
                    if paper.citation_count and paper.citation_count > 0 and paper.independent_citations is not None
                ]) if papers else 0.0
            },
            ...
            'top_independent_papers': sorted([
                {
                    ...
                    'independence_ratio': paper.independent_citations / paper.citation_count if paper.citation_count else 0
                }
                for paper in papers 
                if paper.citation_count and paper.citation_count > 5 and paper.independent_citations
            ], key=lambda x: x['independence_ratio'], reverse=True)[:10]
        }
        
        return report
    
    def _generate_independence_recommendations(self, papers: List[PaperRecord]) -> List[str]:
        ...
        total_papers = len(papers)
        papers_with_citations = [p for p in papers if p.citation_count and p.citation_count > 0]
        
        if not papers_with_citations:
            recommendations.append("Focus on publishing work that attracts independent citations.")
            return recommendations
        
        # Calculate overall metrics
        total_citations = sum(p.citation_count for p in papers_with_citations)
        total_independent = sum(p.independent_citations or 0 for p in papers_with_citations)
        independence_ratio = total_independent / total_citations if total_citations > 0 else 0
